...
![图片](/Users/michael/Project/MichaelBlog/images/redis_all.png)
...
答案：哨兵模式基于主从复制模式，增加了**哨兵来监控**与**自动处理故障**。
...
答案：为了避免数据迁移失效，通常使用`一致性哈希`实现动态扩容缩容，有效减少需要迁移的Key数量。
...
答案：一个redis集群由多个节点node组成，而多个node之间通过`cluster meet`命令来进行连接，组成一个集群。
...
答案：通过`MULTI`、`EXEC`、`WATCH`等命令来实现事务机制，事务执行过程将一系列多个命令按照顺序一次性执行，在执行期间，事务不会被中断，也不会去执行客户端的其他请求，直到所有命令执行完毕。
