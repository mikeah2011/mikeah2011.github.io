...
categories:
- devops
description: 全球化 Laravel 应用多区域部署实战指南，系统覆盖 Active-Active/Active-Passive/混合模式三种架构选型、MySQL
  GTID 跨区域主从复制与 TiDB 分布式数据库方案、CloudFront/Cloudflare Workers 边缘缓存与 Vary 头策略、写后读一致性与最终一致性事件同步、LWW/字段级合并冲突解决、DynamoDB
