...
在单体应用时代，一个请求从进入到结束，所有的日志、错误、性能数据都集中在一个进程内，排障相对简单。而当我们进入微服务架构，一个用户请求可能穿越 5~20 个服务，经过 HTTP、gRPC、消息队列等多种通信协议，传统的日志关联方式彻底失效。

你是否遇到过这样的场景：

- 用户投诉"下单慢"，你只能看到 API Gateway 的响应时间是 3 秒，但无法确定是哪个下游服务拖慢了整条链路？
- 同一笔订单在 A 服务记录了 `order_id=12345`，到了 B 服务的日志里却找不到这个订单号，需要人工逐条匹配时间戳？
- 想基于"用户等级=VIP"来采样高价值用户的链路，却发现这个业务标签根本没有传递到下游服务？

这些问题的根源在于：**你缺少一套标准化的分布式上下文传播机制**。

本文将以 Laravel 微服务架构为实战背景，深入讲解 W3C Trace Context 和 W3C Baggage 两个标准，手把手带你实现从 HTTP 调用到队列任务的全链路上下文传播，并讨论头部采样与尾部采样在实际业务中的选择策略。文章中包含大量可直接用于生产环境的代码示例，以及我们在真实业务中踩过的坑和总结出的最佳实践，希望能帮助正在建设可观测性体系的团队少走弯路。

在正式开始之前，先厘清几个容易混淆的概念。**分布式追踪**的核心目标是记录一个请求在多个服务之间流转的完整路径，包括每一跳的耗时、状态和错误信息。而**上下文传播**则是分布式追踪得以实现的技术基础——它定义了如何将追踪标识（Trace ID、Span ID）和业务标签（用户身份、租户信息等）从一个服务传递到下一个服务。没有正确的上下文传播，追踪系统就无法将散布在不同服务中的 Span 串联成一条完整的链路。这就好比你在快递包裹上贴了追踪号，但中间的转运站不读取这个号码，那这个追踪号就毫无意义。

W3C Trace Context 和 Baggage 正是为了解决"如何标准化地读取和传递这个追踪号"而诞生的国际标准。在它们出现之前，各个 APM 厂商各自为政：Jaeger 使用 `uber-trace-id`，Zipkin 使用 `X-B3-TraceId`，Datadog 使用 `x-datadog-trace-id`。这意味着如果你的系统同时接入了多个 APM 工具，或者需要跨团队、跨公司传递追踪上下文，就会陷入一场"协议翻译"的噩梦。W3C 标准的出现统一了这个局面，让不同的追踪系统可以无缝协作。
