...
![图片](/Users/michael/Project/MichaelBlog/images/6340.png)

InnoDB的主键索引的叶子结点直接就是存储行数据，查找主键索引树就能获得数据：

![图片](/Users/michael/Project/MichaelBlog/images/6240.png)

若是根据非主键索引查找，非主键索引的叶子结点存储的就是，当前索引值以及对应的主键的值，若是联合索引存储的就是联合索引值和对应的主键值。

![图片](/Users/michael/Project/MichaelBlog/images/6140.png)
...
| 崩溃恢复   | 慢，易丢失                                                   | 概率低                                    |
| 索引和数据 | 分开存储                                                     | 存储在一起                                |
| 数据文件   | `.frm`（文件存储表定义）<br />`.MYD` (MYData数据文件)<br />`.MYI` (MYIndex索引文件) | 只受限于操作系统文件的大小<br />一般是2GB |
