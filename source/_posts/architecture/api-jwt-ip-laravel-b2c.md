...
> 当 OWASP Top 10 的基础防护（SQL 注入/XSS/CSRF/SSRF）都做好了之后，API 安全还有哪些容易被忽略的高危攻击面？本文基于 KKday B2C API 真实生产环境的安全加固经验，深入拆解 **JWT 黑名单机制**、**请求签名（HMAC-SHA256）**、**IP 白名单**、**防重放攻击** 四道防线的完整实现与踩坑记录。
...
在 B2C 电商 API 的早期阶段，我们的安全模型很简单：

```
用户登录 → 签发 JWT → 每次请求带 Bearer Token → 后端验证签名
```
...
| 攻击场景 | 说明 | 后果 |
|---------|------|------|
| Token 泄露 | 用户设备被入侵 / 中间人攻击 / 前端日志打印 Token | 攻击者可长期使用被盗 Token |
| Token 无法主动失效 | JWT 是无状态的，签发后无法撤销 | 用户改密码后旧 Token 仍有效 |
| 接口被伪造调用 | 第三方伪造请求格式，直接调用内部 API | 订单/支付接口被恶意调用 |
| 重放攻击 | 攻击者截获合法请求，原封不动重放 | 同一笔支付被重复执行 |
...
│  X-Signature: HMAC-SHA256(body + timestamp + nonce) │
│  X-Timestamp: 1717234567                            │
│  X-Nonce: a8f5f167f44f4964e6c998dee827110c          │
└──────────────────────┬──────────────────────────────┘
                       ▼
              ┌─────────────────┐
  Layer 1 →   │  IP 白名单检查   │  ← 基础设施层，最快拦截
              └────────┬────────┘
                       ▼
              ┌─────────────────┐
  Layer 2 →   │  请求签名校验    │  ← 防伪造、防篡改
              └────────┬────────┘
                       ▼
              ┌─────────────────┐
  Layer 3 →   │  防重放检查      │  ← nonce + timestamp 验证
              └────────┬────────┘
                       ▼
              ┌─────────────────┐
  Layer 4 →   │  JWT 认证 + 黑名单│ ← 身份认证 + 主动失效
              └────────┬────────┘
                       ▼
              ┌─────────────────┐
  Layer 5 →   │  业务逻辑处理    │
...
**核心原则：每一层都能独立拦截攻击，任何单点失效都不会导致全面沦陷。**

### 1.3 JWT 与 Session 的取舍对比

很多团队在做 API 安全时，第一步就会纠结：到底该继续用传统 Session，还是全面切到 JWT？这个问题如果没有结合部署拓扑、登录态撤销需求、网关层能力一起看，很容易做出表面先进、实则难维护的方案。

| 维度 | JWT | Session |
|------|-----|---------|
| 服务端状态 | 默认无状态 | 强依赖服务端状态 |
| 横向扩容 | 天然友好 | 需共享 Session 存储 |
| 主动注销 | 需要黑名单/版本号补偿 | 天然支持 |
| 移动端 / 第三方 API | 更适合 | 不够灵活 |
| 安全审计 | 需额外记录 jti / device_id | 可直接追踪 Session ID |
| 被盗后的止损能力 | 依赖黑名单与短 TTL | 删除 Session 即可 |
| 网关层透传 | 简单 | 常需 Cookie / CSRF 配合 |
| 典型场景 | App、开放平台、前后端分离 API | 管理后台、传统 Web 站点 |

**结论很务实：**

- 如果是纯 API、移动端、第三方接入，JWT 仍然更合适；
- 如果是公司内部后台，Session + SameSite Cookie + CSRF 往往更省心；
