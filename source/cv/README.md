...
./check-content-sync.sh           # 两份内容源的日期是否一致（改日期后必跑）
```
...
5. 改任职日期时只改了 `src/*.md`，`resume.html` 没动 —— 重新导出 12 份 PDF，
   全部还是旧日期，而导出脚本一切正常、零报错 → `check-content-sync.sh`

能生成的就不手写，能检查的就别靠记性 —— 前三个都是「看起来没问题」拖了很久
才发现的，第四、五个更甚：它们连看都看不出来。

第 5 条值得单独说明，因为它是这个结构的固有风险：**内容有两份独立副本** ——
`resume.html` / `deck.html` 里的 i18n 字典（PDF 和线上版都由它生成），以及
`src/*.md`（源文件预览页读的）。两者没有生成关系，改一处不会同步另一处。

合并成一份内容源当然更好，但那需要一套构建流程，而这个站刻意保持「无构建、
纯静态、`skip_render` 直接发布」。所以选择保留两份 + 一个检查脚本 —— 这是知情的
取舍。`check-content-sync.sh` 只比对日期区间：那是最常改、最容易漏、写错后果
最严重的字段（履历日期出错是硬伤），全文比对会淹没在措辞差异的噪音里。
