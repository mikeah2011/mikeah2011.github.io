...
- **稳定性与团队**：主导大规模故障救火（关键 API p99 25s+ → <2s，5xx 稳定 <0.01%）；现兼任售前 CVR 增转专项组负责人，统筹 6 人跨端小组

---

## AI 工程化实践

- **Maestro**（TypeScript CLI）——**多 AI 审查机器人收敛层**：编排 CodeRabbit / Copilot / Codex / Gemini 4 个机器人，经问题分类 → 三层去重 → 自动修复 → 回复并解决讨论串 → 收敛监控与人工升级判断，把发散的 AI 意见收敛成可执行的修改队列；**已成为团队日常流程**：10 人使用、累计走过 300+ 个 PR；据团队自评，Code Review 耗时**明显缩短**、误报率下降约 70%
...
全球领先的旅游体验预订平台（覆盖 50+ 国家/地区）。近四年历任联盟营销平台负责人、B2C 主干线核心后端，现兼任售前 CVR 增转专项组负责人（当前约 80% 精力投入于此，另 20% 用于 B2C 核心链路收尾交接）。
