...
**资深后端工程师 · 13 年研发经验 · 高并发交易系统 / 分布式架构**
...
13 年后端研发经验，专注**高并发电商交易系统**与**分布式架构**，横跨 OTA / 电商 / MarTech 三个业务场景。擅长在真实生产压力下做架构决策：救过大面积线上故障，也独立完成过从熔断器设计到吞吐量 17 倍提升的性能优化。

**13 年** — 后端研发 · OTA / 电商 / MarTech  
**¥100 亿** — Apple 中国经销商平台 5 个月 GMV  
**17.5 倍** — H5 接口吞吐提升（200 → 3500 TPS）  
**25s → 2s** — 大促故障救火后关键 API p99  
**200%** — KKday 联盟业务年同比增长  
**65%↓** — 联盟平台 API 延迟优化（120ms → 35ms）

- **交易系统纵深**：以第一贡献人身份主导 KKday 联盟营销平台（业务年同比增长 200%），并负责 B2C 主干线核心交易链路，横跨订单、支付回调、票券架构
- **高并发与稳定性**：主导大规模故障救火（关键 API p99 25s+ → <2s，5xx 稳定 <0.01%）；Apple 中国经销商平台 H5 接口吞吐量提升 17.5 倍，压测调优达 12K TPS
- **跨团队技术影响力**：主笔 7 份 SA/SD 推动 B2C/Order/MKT/Member 四个服务团队一次对齐；建立「BFF 层版本兼容 + App 渐进升级」标准解法被团队沿用；担任联盟平台 Code Owner 与 PR 把关人
...
| AI 工程（延伸） | Claude API 生产化落地、多 AI Agent 编排与收敛 |
...
全球领先的旅游体验预订平台（覆盖 50+ 国家/地区）。近四年历任联盟营销平台负责人、B2C 主干线核心后端，现阶段主要负责售前 CVR 增转的技术实现与监控看板搭建（约 80% 精力），同步收尾并交接 B2C 核心链路（约 20% 精力）。

**1) 售前 CVR 增转专项组（2026 H2）**
- 主导售前链路（商品页 → 购物车 → 订购页）全端性能优化，以「CVR 1.2% → 1.8%（+50%）」为技术目标；搭建 CVR 健康监控看板，量化每次优化的实际效果

**2) B2C 核心交易链路（2025.02 – 2026.08）**
- **USJ（大阪环球影城）订单改期**：跨 B2C / Order / MKT / Member 四大服务改造，主笔 7 份 SA/SD；提出「权益继承、条件重审」原则，将散乱的优惠券/积分重验规则整理为可审计决策表；重构金额试算器与订单状态机，保障公司 TOP 级票券供应商续约
- **大面积故障救火**：定位微服务依赖拖垮 PHP-FPM worker 池的根因，7 天内完成 P0+P1 加固（超时优化、异常兜底、Redis 轻量熔断器、双层 Feature Flag / Kill Switch）——cart/validate API p99 由 25s+ 降至 <2s，根治 iOS 下单白屏，此后 5xx 错误率连续多周 <0.01%
- **票券/Tour 商品架构升级**：主导方案分组、组合商品、OpenDate 月历下放等专案，独立完成 SA/SD → Mock → API → 单测 → 文档全流程；建立「BFF 层版本兼容 + App 渐进升级」标准解法，修复 15+ 跨端差异场景

**3) 联盟营销平台（2022.11 – 2025.03）**
...
- **Apple 中国经销商平台**（该平台上线 5 个月 GMV 超 ¥100 亿）：本人主责预售、交易、支付回调、配送发货核心链路的架构设计与实现，并负责发版与服务器运维；串接顺丰、宝尊 OMS 等 15+ 支付/物流系统，服务 3500+ 零售门店、25 万+ DAU
